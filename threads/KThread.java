...
public class KThread {
	...
	static private KThread A, B, C;
	static private void runA() {
		for (int i = 0; i < 10; ++i) {
			System.out.println("*** thread 1 looped " + i + " times");
			yield();
		}
	}
	static private void runB() {
		for (int i = 0; i < 5; ++i) {
	...
	static private void testProblem1() {
	...
	static private Lock mutex;
	static private Condition2 condition;
	static private int flag = 0;
	static private KThread D, E;
	static private void runD() {
		for (int i = 0; i < 10; ++i) {
		mutex.acquire();
		while (flag == 1) {
			condition.sleep();
		...
		flag = flag + 1;
		condition.wake();
		System.out.println("Hello, I am thread D");
		mutex.release();
		}
	}
	static private void runE() {
		for (int i = 0; i < 10; ++i) {
		mutex.acquire();
		while (flag == 0) {
			condition.sleep();
		...
		flag = flag - 1;
		condition.wake();
		System.out.println("Hello, I am thread E");
		mutex.release();
		}
	}
	static private void testProblem2() {
		mutex = new Lock();
		condition = new Condition2(mutex);
		D = new KThread(new Runnable() {
				public void run() {
					runD();
				}
			});
		E = new KThread(new Runnable() {
				public void run() {
					runE();
				}
			});
	...
	static private KThread F;
	static private void runF() {
		System.out.println("Hello, I am going to wait!");
		System.out.println(Machine.timer().getTime());
		ThreadedKernel.alarm.waitUntil(10);
		System.out.println(Machine.timer().getTime());
		System.out.println("I am done!");
	}
	static private void testProblem3() {
		F = new KThread(new Runnable() {
				public void run() {
					runF();
				}
			});
	...
	static private KThread G, H;
	static private Communicator communicator;
	static private void runG() {
		System.out.println("G: I speak 1.");
		communicator.speak(1);
		System.out.println("G: I am done.");
	}
	static private void runH() {
		System.out.println("H: I listen.");
    ...
    static private void runK() {
        ...
        System.out.println("K: Efficient value"+((PriorityScheduler)ThreadedKernel.scheduler).getEffectivePriority(K));
        System.out.println("J: Efficient value"+((PriorityScheduler)ThreadedKernel.scheduler).getEffectivePriority(J));
        System.out.println("L: Efficient value"+((PriorityScheduler)ThreadedKernel.scheduler).getEffectivePriority(L));
        System.out.println("I: Efficient value"+((PriorityScheduler)ThreadedKernel.scheduler).getEffectivePriority(I));
